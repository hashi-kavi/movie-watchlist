...
export default function MovieCard({ movie, onAdd, onRemove, isInWatchlist }){
  ...
  const initialRating = movie.rating ?? movie.vote_average ?? 0
    ...
    >
      <div className="relative w-full aspect-[2/3]">
        ...
        <div className="absolute top-2 right-2 bg-black/80 backdrop-blur-sm px-2 py-1 rounded-lg flex items-center gap-1">
          ...
          <span className="text-xs font-semibold text-white">{(userRating ?? 0).toFixed ? (userRating ?? 0).toFixed(1) : (userRating ?? 0)}</span>
        </div>
      ...
      <div className="absolute top-2 left-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        ...
        {isInWatchlist && (
          <div className="flex items-center gap-2 bg-black/60 px-2 py-1 rounded-md">
            <span className="text-xs text-gray-200">Rate</span>
            <input
              type="range"
              min="0"
              max="10"
            ...
            />
            <span className="text-xs text-gray-200">{(userRating ?? 0).toFixed ? (userRating ?? 0).toFixed(1) : (userRating ?? 0)}</span>
          </div>
        )}
      </div>
    </motion.div>
