...
router.put('/rating', async (req, res) => {
  try {
    ...
    if (tmdbId === undefined || tmdbId === null) return res.status(400).json({ message: 'tmdbId required' });

    const r = rating === undefined || rating === null ? undefined : Number(rating);
    if (r !== undefined && (Number.isNaN(r) || r < 0 || r > 10)) {
